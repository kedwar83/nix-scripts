...
setup_distrobox() {
   ...
   distrobox enter --name debian-distro -- bash -c '
    sudo apt update &&
    sudo apt install -y virt-manager curl gnupg apt-transport-https &&
    # Install Signal:
    wget -O- https://updates.signal.org/desktop/apt/keys.asc | sudo gpg --dearmor > /usr/share/keyrings/signal-desktop-keyring.gpg &&
    cat /usr/share/keyrings/signal-desktop-keyring.gpg | sudo tee /etc/apt/sources.list.d/signal-xenial.list > /dev/null &&
    sudo apt update &&
